...
import (
	...
	"github.com/metacubex/mihomo/transport/shieldlink"
)
...
func ParseProxy(mapping map[string]any, options ...ProxyOption) (C.Proxy, error) {
	...
	// ShieldLink: intercept BEFORE proxy construction.
	// Start an embedded local listener and override server/port so the proxy
	// connects through the ShieldLink tunnel transparently.
	if slMapping, slExist := mapping["shieldlink"].(map[string]any); slExist {
		slCfg := &shieldlink.ClientConfig{}
		if err := decoder.Decode(slMapping, slCfg); err != nil {
			return nil, fmt.Errorf("shieldlink config: %w", err)
		}
		if slCfg.UUID != "" && len(slCfg.Servers) > 0 {
			host, port, err := shieldlink.StartEmbeddedListener(slCfg)
			if err != nil {
				return nil, fmt.Errorf("shieldlink listener: %w", err)
			}
			// Override the proxy's target to our local tunnel endpoint
			mapping["server"] = host
			mapping["port"] = port
		}
	}
	...
	case "shadowveil":
		shadowVeilOption := &outbound.ShadowVeilOption{BasicOption: basicOption}
		err = decoder.Decode(mapping, shadowVeilOption)
		if err != nil {
			break
		}
		proxy, err = outbound.NewShadowVeil(*shadowVeilOption)
	default:
